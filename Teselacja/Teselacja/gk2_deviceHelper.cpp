...
shared_ptr<ID3DBlob> DeviceHelper::CompileD3DShader(const wstring& filePath, const string& entry,
													const string& shaderModel, const D3D10_SHADER_MACRO* defines)
{
	...
	HRESULT result = D3DX11CompileFromFileW(filePath.c_str(), defines, 0, entry.c_str(), shaderModel.c_str(), shaderFlags,
		0, 0, &b, &eb, 0);
...
shared_ptr<ID3D11HullShader> DeviceHelper::CreateHullShader(shared_ptr<ID3DBlob> byteCode)
{
	assert(m_deviceObject);
	ID3D11HullShader* hs;
	HRESULT result = m_deviceObject->CreateHullShader(byteCode->GetBufferPointer(),
													  byteCode->GetBufferSize(), 0, &hs);
	shared_ptr<ID3D11HullShader> hullShader(hs, Utils::COMRelease);
	if (FAILED(result))
		THROW_DX11(result);
	return hullShader;
}

shared_ptr<ID3D11DomainShader> DeviceHelper::CreateDomainShader(shared_ptr<ID3DBlob> byteCode)
{
	assert(m_deviceObject);
	ID3D11DomainShader* ds;
	HRESULT result = m_deviceObject->CreateDomainShader(byteCode->GetBufferPointer(),
														byteCode->GetBufferSize(), 0, &ds);
	shared_ptr<ID3D11DomainShader> domainShader(ds, Utils::COMRelease);
	if (FAILED(result))
		THROW_DX11(result);
	return domainShader;
}
