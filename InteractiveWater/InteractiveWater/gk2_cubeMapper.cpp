...
const wstring CubeMapper::ShaderFile = L"resources/shaders/CubeMapShader.hlsl";
const int CubeMapper::TEXTURE_SIZE = 256;

CubeMapper::CubeMapper(DeviceHelper& device, shared_ptr<ID3D11InputLayout>& layout,
...
void CubeMapper::InitializeTextures(DeviceHelper& device)
{
...
void CubeMapper::SetupFace(const shared_ptr<ID3D11DeviceContext>& context, D3D11_TEXTURECUBE_FACE face)
{
...
void CubeMapper::SetSamplerState(const shared_ptr<ID3D11SamplerState>& samplerState)
{
	if (samplerState != nullptr)
		m_samplerState = samplerState;
}

void CubeMapper::SetCameraPosBuffer(const shared_ptr<ConstantBuffer<XMFLOAT4>>& cameraPos)
{
	if (cameraPos != nullptr)
		m_cameraPosCB = cameraPos;
}

void CubeMapper::SetSurfaceColorBuffer(const shared_ptr<ConstantBuffer<XMFLOAT4>>& surfaceColor)
{
	if (surfaceColor != nullptr)
		m_surfaceColorCB = surfaceColor;
}

void CubeMapper::SetVertexShaderData()
{
...
void CubeMapper::SetPixelShaderData()
{
