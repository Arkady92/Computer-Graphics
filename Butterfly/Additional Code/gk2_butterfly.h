...
namespace gk2
{
	...
	private:
		...
		std::shared_ptr<ID3D11VertexShader> m_vertexShader2;
		std::shared_ptr<ID3D11PixelShader> m_pixelShader2;
		//VertexPosNormal input layout
		std::shared_ptr<ID3D11InputLayout> m_inputLayout2;
		...
		//Wing's vertex buffer
		std::shared_ptr<ID3D11Buffer> m_vbBillboard;
		//Wing's index buffer
		std::shared_ptr<ID3D11Buffer> m_ibBillboard;
		...
		std::shared_ptr<ID3D11BlendState> m_bsBillboard;
