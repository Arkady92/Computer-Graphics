...
float4 PS_Main(PSInput i) : SV_TARGET
{
	...
	float4 lightColor = lightMap.Sample(colorSampler, float2(position.x, position.y));
	float shadowZ = shadowMap.Sample(colorSampler, float2(position.x, position.y));
	if (lightColor.x < defLightColor.x || i.worldPos.y > lightPosPS.y || shadowZ < position.z)
